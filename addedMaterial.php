  ...
  <div class="site-wrap">
    ...
    <div id="container" align="center">
        ...
        //Connect to Database
         ...
         //use database
         $use1DB = " use earthprotectors";
           
         $conn->query($use1DB);
    

         $showquery = "SELECT colmaterial.materialID, material.materialName,material.description,material.pointsPerKg 
         from material INNER JOIN colmaterial ON colmaterial.materialID=material.materialID Where colmaterial.username='$user';";
         ...
         //create array to store selected fields data
               ...
               if($result->num_rows > 0){
                   ...
                   $colmaterialArr[] = array('mId'=> $row['materialID'], 'name' => $row['materialName'],
                   'des' => $row['description'], 'points' => $row['pointsPerKg']);
                   }
                }

       echo '<h4><b>Material Added</b></h4>';
       ...
       foreach($colmaterialArr as $colmaterialArr){
           echo'<tr>'; 
                       echo'<td>'. $colmaterialArr['mId'].'</td>';
                       echo'<td>'. $colmaterialArr['name'].'</td>';
                       echo'<td>'. $colmaterialArr['des'].'</td>';
           echo'<td>'. $colmaterialArr['points'].'</td>';
           echo'</tr>';
         }  
